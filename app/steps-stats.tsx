...
export default function StepsStatsScreen() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await statisticsService.getStepsStats(range);
            setChartData(data?.chartData ?? []);
            setAverage(data?.average ?? 0);
            setTotal(data?.total ?? 0);
            setMax(data?.max ?? 0);
        } catch (error) {
            console.error('Failed to load steps stats:', error);
            setChartData([]);
            setAverage(0);
            setTotal(0);
            setMax(0);
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.header}>
                ...
                <Text style={[styles.headerTitle, { color: colors.text }]}>Steps Statistics</Text>
                ...
                <>
                    ...
                    <View style={styles.summaryContainer}>
                        <View style={[styles.summaryCard, { backgroundColor: colors.cardBackground }]}>
                            ...
                            <Text style={[styles.summaryUnit, { color: colors.textSecondary }]}>steps</Text>
                        </View>
                        <View style={[styles.summaryCard, { backgroundColor: colors.cardBackground }]}>
