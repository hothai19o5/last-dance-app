...
export default function WeightStatsScreen() {
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const data = await statisticsService.getWeightStats(range);
    ...
    return (
        <View style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.header}>
                ...
                <Text style={[styles.headerTitle, { color: colors.text }]}>Weight Statistics</Text>
                ...
                <>
                    ...
                    <View style={styles.summaryContainer}>
                        <View style={[styles.summaryCard, { backgroundColor: colors.cardBackground }]}>
                            ...
                            <Text style={[styles.summaryValue, { color: colors.text }]}>{average.toFixed(1)}</Text>
                            <Text style={[styles.summaryUnit, { color: colors.textSecondary }]}>kg</Text>
                        </View>
                        <View style={[styles.summaryCard, { backgroundColor: colors.cardBackground }]}>
                            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Min</Text>
                            <Text style={[styles.summaryValue, { color: colors.text }]}>{min.toFixed(1)}</Text>
                            <Text style={[styles.summaryUnit, { color: colors.textSecondary }]}>kg</Text>
                        </View>
                        <View style={[styles.summaryCard, { backgroundColor: colors.cardBackground }]}>
                            <Text style={[styles.summaryLabel, { color: colors.textSecondary }]}>Max</Text>
                            <Text style={[styles.summaryValue, { color: colors.text }]}>{max.toFixed(1)}</Text>
                            <Text style={[styles.summaryUnit, { color: colors.textSecondary }]}>kg</Text>
                        </View>
                    </View>
                    ...
                    <View style={[styles.chartCard, { backgroundColor: colors.cardBackground }]}>
                        ...
                        <View style={styles.chartContainer}>
                            <LineChart
                                ...
                                curved
                                thickness={2}
                                color={colors.weightColor || colors.warning}
                                yAxisThickness={0}
                                xAxisThickness={1}
